...
JNIEXPORT void JNICALL Java_jxgrabkey_JXGrabKey_clean(JNIEnv *_env, jobject _obj) {
...
JNIEXPORT void JNICALL Java_jxgrabkey_JXGrabKey_registerHotkey__III(JNIEnv *_env, jobject _obj, jint _id, jint _mask, jint _key) {
	if (debug) {
		ostringstream sout;
	...
	if (registerHotkeyHasException) {
		...
		if (cls != NULL) {
			...
			//Don't print anything to console until function return, or else java will not register the exception!
		} else {
			if (debug) {
				ostringstream sout;
...
JNIEXPORT void JNICALL Java_jxgrabkey_JXGrabKey_unregisterHotKey(JNIEnv *_env, jobject _obj, jint _id) {
	if (debug) {
		ostringstream sout;
...
JNIEXPORT void JNICALL Java_jxgrabkey_JXGrabKey_listen(JNIEnv *_env, jobject _obj) {
	if (debug) {
		ostringstream sout;
		sout << "++ listen()";
		printToDebugCallback(_env, sout.str().c_str());
	...
	while (doListen) {
		while (!XPending(dpy) && doListen) {
		    //Don't block on XNextEvent(), this breaks XGrabKey()!
			usleep(SLEEP_TIME_MS * MICROSECOND_TO_MILLISECOND_MULTIPLICATOR);
		}
...
JNIEXPORT void JNICALL Java_jxgrabkey_JXGrabKey_setDebug(JNIEnv *_env, jobject _obj, jboolean _debug) {
	debug = _debug;
}
...
void getOffendingModifiers(Display* _dpy) {
	...
	KeyCode nlock, slock;
	static int mask_table[8] = { ShiftMask, LockMask, ControlMask, Mod1Mask, Mod2Mask, Mod3Mask, Mod4Mask, Mod5Mask };
...
static int *xErrorHandler(Display *_dpy, XErrorEvent *_event) {
	if (registerHotkeyIsWaitingForException) {
		//The exception has to be thrown in registerHotkey, bcoz this functions runs in a different thread!
...
void grabKey(JNIEnv *_env, KeyStruct key) {
	...
	for (int screen = 0; screen < ScreenCount(dpy); screen++) {
		for (int m = 0; m < 8; m++) {
			...
			if (debug && !ret) {
				ostringstream sout;
				sout << "grabKey() - WARNING: XGrabKey() on screen " << std::dec << screen << " for mask combination " << std::dec << m << " returned false";
				printToDebugCallback(_env, sout.str().c_str());
			}
		}
	}
...
void ungrabKey(JNIEnv *_env, KeyStruct key) {
	...
	for (int screen = 0; screen < ScreenCount(dpy); screen++) {
		for (int m = 0; m < 8; m++) {
			...
			if (debug && !ret) {
				ostringstream sout;
				sout << "ungrabKey() - WARNING: XUngrabKey() on screen " << std::dec << screen << " for mask combination " << std::dec << m << " returned false";
				printToDebugCallback(_env, sout.str().c_str());
			}
		}
	}
